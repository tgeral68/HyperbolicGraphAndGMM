...
for directory in list_of_directory:
    if(args.fls not in directory and args.fls!='*'):
        ...
        try:
            ...
            ll = logger_object["unsupervised_eval_k_means"]["nmi"]
            if(ll[0] == 0.):
            ...
            ll = logger_object["unsupervised_eval_k_means"]["accuracy"]
            if("accuracy" not in group[dct_key]):
                group[dct_key]["accuracy"] = []
            group[dct_key]["accuracy"] += ll

            ll = logger_object["unsupervised_eval_k_means"]["conductance"]
